...
The `index.js` script ensures that all `SKILL.md` files conform to the standard format. Specifically, it checks that the YAML frontmatter includes the following fields:

- `name` (required)
- `description` (required)
- `metadata` (outputs a warning if missing, does not fail the build)
- `version` (outputs a warning if missing, does not fail the build)
