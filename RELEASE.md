...
Skills 文件托管在阿里云 OSS 上，供 `npx skills add` 通过 well-known endpoint 协议下载。
...
开发者 make oss-sync
    ...
    │    3. 生成 .well-known/skills/       │
    │       index.json (自动)              │
    │    4. ossutil cp 上传到 OSS           │
    └──────────────────────────────────────┘
           │
           ▼
    oss://adex-skills/.well-known/skills/
      ├── index.json
...
| 组件 | 文件 | 作用 |
|------|------|------|
| OSS 同步脚本 | `scripts/oss-sync.sh` | 生成 index.json + 上传 skills 到 OSS |
| Makefile target | `Makefile` (`oss-sync`) | `make oss-sync` 入口 |
| OSS 凭证 | `.env` | `OSS_ACCESS_KEY_ID` / `OSS_ACCESS_KEY_SECRET`（gitignore） |
| 安装向导 | `scripts/install-wizard.js` | 安装时自动调用 `npx skills add` 从 OSS 下载 |

### .env 配置

`.env` 文件（已 gitignore）包含 OSS 凭证，格式如下：

```
OSS_ACCESS_KEY_ID=LTAIxxxxxxxxxxxx
OSS_ACCESS_KEY_SECRET=xxxxxxxxxxxxxxxxxxxx
OSS_BUCKET=adex-skills
OSS_ENDPOINT=oss-cn-hangzhou.aliyuncs.com
```

### 更新 Skills

当 `skills/` 目录下的 SKILL.md 或 references 文件有变更时，运行：
...
脚本会自动：
1. 扫描 `skills/*/SKILL.md` 的 frontmatter（name、description）
2. 遍历每个 skill 目录收集文件列表
3. 生成 `index.json`
4. 用 ossutil 上传到 `oss://adex-skills/.well-known/skills/`

### 验证上传

```bash
# 检查 index.json
curl -s https://adex-skills.oss-cn-hangzhou.aliyuncs.com/.well-known/skills/index.json | python3 -m json.tool
...
- **ossutil** 已安装（v1.7.x）
  - macOS: `curl -L -o /tmp/ossutil.zip https://gosspublic.alicdn.com/ossutil/1.7.19/ossutil-v1.7.19-mac-amd64.zip && unzip -o /tmp/ossutil.zip -d ~/bin/`
  - 其他平台: https://help.aliyun.com/zh/oss/developer-reference/install-ossutil
- **.env** 文件已配置 OSS 凭证
- **OSS Bucket** `adex-skills` 已创建并设置为公共读
- **python3** 可用（用于生成 index.json）

### ⚠️ .env 不要提交到 git

`.env` 包含 OSS AccessKey，已在 `.gitignore` 中。切勿提交。
