...
func clearSkipEnv(t *testing.T) {
	t.Helper()
	for _, key := range []string{"ADEX_NO_UPDATE_NOTIFIER", "CI", "BUILD_NUMBER", "RUN_ID"} {
		t.Setenv(key, "")
		os.Unsetenv(key)
	}
}
...
func TestShouldSkip(t *testing.T) {
	tests := []struct {
		...
		{"notifier_off", "1.0.0", map[string]string{"ADEX_NO_UPDATE_NOTIFIER": "1"}, true},
...
func TestUpdateInfoMethods(t *testing.T) {
	...
	got := info.Message()
	want := "adex 2.0.0 available, current 1.0.0, run: adex update"
	if got != want {
