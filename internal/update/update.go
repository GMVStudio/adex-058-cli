...
const (
	registryURL  = "https://registry.npmjs.org/@gmvstudio/adex-cli/latest"
...
func (u *UpdateInfo) Message() string {
	return fmt.Sprintf("adex %s available, current %s, run: adex update", u.Latest, u.Current)
}
...
func shouldSkip(version string) bool {
	if os.Getenv("ADEX_NO_UPDATE_NOTIFIER") != "" {
		return true
	}
	if IsCIEnv() {
		return true
...
func saveState(s *updateState) error {
	...
	return vfs.Default.WriteFile(statePath(), data, 0o644)
}
