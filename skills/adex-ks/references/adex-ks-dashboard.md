...
租户级别的快手广告投放概览，一次调用返回账户统计、日期范围内消耗汇总、以及账户消耗排名。对应 API：`GET /v1/ks/dashboard`。
...
# 显式日期范围
adex ks dashboard --begin 2026-06-01 --end 2026-06-30 --ranking-limit 10

# 最近 7 天概览，排名 Top 20
adex ks dashboard --range 7d --ranking-limit 20

# 按数据源筛选
adex ks dashboard --range 30d --source "ks"

# pretty 格式输出
adex ks dashboard --range 30d --format pretty

# 提取消耗汇总
adex ks dashboard --range 30d --jq '.charge'

# 预览请求但不执行
adex ks dashboard --range 30d --dry-run
```
...
| `--source` | string | — | 数据源过滤 |
| `--ranking-limit` | int | 10 | 账户排名数量（最大 100） |
...
- **作为下钻起点**：先 dashboard 看大盘 → 再 `account-reports daily` 看趋势 → 再 `campaigns top` 看排名

## 注意事项

- `dashboard` 返回的是单个对象，不是列表，因此**不支持** `--page-size` / `--page-token` / `--page-all`
- `--ranking-limit` 控制返回的账户排名数量，默认 10，最大 100
- 日期范围为**必需**参数，不传会返回验证错误

## 参考

- [adex-ks](../SKILL.md) — 快手广告全部命令
- [adex-ks-accounts](adex-ks-accounts.md) — 账户列表
- [adex-ks-reports](adex-ks-reports.md) — 日报表和汇总报表
- [adex-ks-workflow-top-analysis](adex-ks-workflow-top-analysis.md) — Top-N 分析工作流
